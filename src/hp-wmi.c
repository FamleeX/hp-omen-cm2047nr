...
/* DMI board names of Omen laptops with four-zone RGB keyboards. */
static const char * const omen_fourzone_rgb_boards[] = {
	"8BB0", /* placeholder/fallback: is_omen_thermal_profile() also enables RGB */
};
...
/* Support for the HP Omen four-zone keyboard lighting. */
...
static bool is_omen_fourzone_rgb_supported(void)
{
	const char *board_name = dmi_get_system_info(DMI_BOARD_NAME);
	...
	if (match_string(omen_fourzone_rgb_boards,
			 ARRAY_SIZE(omen_fourzone_rgb_boards),
			 board_name) >= 0)
		return true;

	/* Keep RGB working on already-recognized Omen boards too. */
	return is_omen_thermal_profile();
}
...
static void hp_wmi_notify(union acpi_object *obj, void *context)
{
	...
	case HPWMI_FN_P_HOTKEY:
		if (!sparse_keymap_report_event(hp_wmi_input_dev, 0x20e6, 1, true))
			pr_info("Unknown Fn+P key code - 0x%x\n", 0x20e6);
		break;
	case HPWMI_OMEN_KEY:
...
static ssize_t zone_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (!target_zone)
		return sysfs_emit(buf, "red: -1, green: -1, blue: -1\n");

	ret = fourzone_update_led(target_zone, HPWMI_READ);
	if (ret)
		return sysfs_emit(buf, "red: -1, green: -1, blue: -1\n");

	return sysfs_emit(buf, "red: %d, green: %d, blue: %d\n",
...
static void fourzone_cleanup(struct platform_device *dev)
{
	u8 zone;
	...
	if (zone_dev_attrs) {
		for (zone = 0; zone < FOURZONE_COUNT; zone++) {
			...
			zone_dev_attrs[zone].attr.name = NULL;
		}
...
static int fourzone_setup(struct platform_device *dev)
{
	...
	if (!is_omen_fourzone_rgb_supported())
		return 0;

	zone_dev_attrs = kcalloc(FOURZONE_COUNT + 1, sizeof(*zone_dev_attrs), GFP_KERNEL);
	if (!zone_dev_attrs)
		return -ENOMEM;

	zone_attrs = kcalloc(FOURZONE_COUNT + 1, sizeof(*zone_attrs), GFP_KERNEL);
	if (!zone_attrs) {
		ret = -ENOMEM;
		goto err_cleanup;
	}

	zone_data = kcalloc(FOURZONE_COUNT, sizeof(*zone_data), GFP_KERNEL);
	if (!zone_data) {
		ret = -ENOMEM;
		goto err_cleanup;
	}
	...
	for (zone = 0; zone < FOURZONE_COUNT; zone++) {
		snprintf(buffer, sizeof(buffer), "zone%02hhX", zone);
...
static int __init hp_wmi_bios_setup(struct platform_device *device)
{
	...
	if (err < 0)
		pr_warn("four-zone RGB setup failed: %d\n", err);

	err = macro_key_setup(device);
	if (err < 0)
		pr_warn("macro key setup failed: %d\n", err);
